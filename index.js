...
for (let i = 0; i < finances.length - 1; i++) {
    monthlyChanges = monthlyChanges + ((finances [i+1][1]) - (finances[i][1]));
};

console.log(monthlyChanges); 

Average = monthlyChanges/(finances.length-1); 
...
for (let i = 0; i < finances.length - 1; i++) {
    currentGreatestIncreaseProfits = ((finances [i+1][1]) - (finances[i][1])); 
    if (currentGreatestIncreaseProfits > greatestIncreaseProfits) { 
        greatestIncreaseProfits = currentGreatestIncreaseProfits;
...
for (let i = 0; i < finances.length - 1; i++) {
    currentGreatestDecreaseProfits = ((finances [i+1][1]) - (finances[i][1])); 
    if (currentGreatestDecreaseProfits <  greatestDecreaseProfits) {  
        greatestDecreaseProfits = currentGreatestDecreaseProfits;
        indexOfGreatestProfit = i+1;
    }
